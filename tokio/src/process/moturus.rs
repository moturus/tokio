...
pub(crate) fn spawn_child(cmd: &mut std::process::Command) -> io::Result<SpawnedChild> {
    let mut std_child = cmd.spawn()?;
    let stdin = std_child.stdin.take().map(stdio).transpose()?;
    let stdout = std_child.stdout.take().map(stdio).transpose()?;
    let stderr = std_child.stderr.take().map(stdio).transpose()?;

    use std::os::moturus::ChildExt;
    let child_fd: RawFd = moto_rt::fs::open(
        format!("handle://{}", std_child.sys_handle()).as_str(),
        moto_rt::fs::O_HANDLE_CHILD,
    )
    .map_err(std::os::moturus::map_moturus_error)?;
